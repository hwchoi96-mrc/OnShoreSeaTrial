  ...
  <div class="equipment-container" :style="iconStyle">
    <div class="equipment-area">
      ...
      >
        ...
        <v-card-text class="d-flex flex-wrap justify-center flex-column">
          <template v-for="(obj, obj_index) in props.list" :key="obj_index">
            <v-sheet class="d-flex justify-space-between lcc-default-font mb-2">
              <div class="text-secondary lcc-sub-font mr-6">{{ obj.key }}</div>
              <div>
                {{ obj.value }}
                <span class="text-secondary lcc-sub-font">{{ obj.unit }}</span>
              </div>
            </v-sheet>
          </template>
        </v-card-text>
