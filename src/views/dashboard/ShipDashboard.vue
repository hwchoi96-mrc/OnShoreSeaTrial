<template>
  <v-container style="height: 100vh; max-height: calc(100vh - 65px)" fluid>
    <v-row>
          ...
          <v-card class="rounded-lg d-flex flex-column" style="flex: 3" color="#333334">
            ...
            <v-card-text class="d-flex ga-2">
              <div class="d-flex flex-column align-center w-50">
                ...
                <v-sheet
                  class="d-flex flex-column justify-center align-center flex-grow-1 ga-2 w-100 py-2"
                  color="#030914"
                >
                  <div class="lcc-default-font text-primary">
              ...
              <div class="d-flex flex-column align-center w-50">
                ...
                <div class="w-100" style="flex-grow: 2">
                  <EChart
                    :option="generatorEngineChartOption"
                    :style="mainEngineChartOption.style"
                  />
                </div>

                <v-sheet
                  class="d-flex flex-column justify-center align-center flex-grow-1 ga-2 w-100 lcc-default-font py-2"
                  color="#030914"
                >
      ...
      <v-col cols="8" lg="8" class="pa-4 overflow-hidden">
...
import { openNewPopup, displayOnlySuperAdmin } from '@/composables/util.js'
  ...
  const position = () => {
    if (type == 'LNG') {
      switch (tab.value) {
        case 'iso':
          return {
            ...
            engine: { image: [73, 44], list: [-8, -17], align: 'right' },
            propeller: { image: [87, 35], list: [0, 6], align: 'top' }
          }
        case 'side':
          return {
            voyage: { image: [72, 46], list: [0, -8], align: 'top' },
            fuel: { image: [38, 57], list: [20, -10], align: 'top' },
            engine: { image: [60, 57], list: [40, 0], align: 'bottom' },
            propeller: { image: [95, 62], list: [0, 0], align: 'bottom' }
          }
        case 'rear':
          return {
            voyage: { image: [60, 38], list: [-15, 0], align: 'left' },
            fuel: { image: [83, 45], list: [-15, 28], align: 'right' },
            engine: { image: [65, 61], list: [6, -3], align: 'right' },
            propeller: { image: [25, 82], list: [-30, -10], align: 'bottom' }
          }
      }
    } else if (type == 'BULK') {
      switch (tab.value) {
        case 'iso':
          return {
            voyage: { image: [63, 31], list: [-10, 0], align: 'left' },
            fuel: { image: [56, 53], list: [0, 10], align: 'bottom' },
            engine: { image: [66, 45], list: [0, -15], align: 'right' },
            propeller: { image: [87, 28], list: [0, 6], align: 'top' }
          }
        case 'side':
          return {
            voyage: { image: [76, 48], list: [0, -6], align: 'top' },
            fuel: { image: [45, 55], list: [20, -10], align: 'top' },
            engine: { image: [27, 55], list: [0, 0], align: 'bottom' },
            propeller: { image: [91, 60], list: [0, 0], align: 'bottom' }
          }
        case 'rear':
          return {
            voyage: { image: [60, 42], list: [-15, 0], align: 'left' },
            fuel: { image: [83, 45], list: [-15, 28], align: 'right' },
            engine: { image: [60, 61], list: [8, 0], align: 'right' },
            propeller: { image: [18, 79], list: [0, 0], align: 'right' }
          }
      }
      ...
      switch (tab.value) {
        case 'iso':
          return {
            voyage: { image: [55, 35], list: [-10, 0], align: 'left' },
            fuel: { image: [50, 65], list: [0, 0], align: 'bottom' },
            engine: { image: [66, 52], list: [0, -15], align: 'right' },
            propeller: { image: [87, 35], list: [0, 2], align: 'top' }
          }
        case 'side':
          return {
            voyage: { image: [57, 48], list: [20, -6], align: 'top' },
            fuel: { image: [27, 55], list: [20, -10], align: 'left' },
            engine: { image: [71, 55], list: [0, 0], align: 'bottom' },
            propeller: { image: [97, 55], list: [0, -10], align: 'top' }
          }
        case 'rear':
          return {
            voyage: { image: [60, 42], list: [35, 0], align: 'top' },
            fuel: { image: [83, 45], list: [-15, 28], align: 'right' },
            engine: { image: [60, 61], list: [-25, 0], align: 'left' },
            propeller: { image: [30, 79], list: [20, 0], align: 'right' }
          }
      }
    }
  }
...
const fetchDashboardShipInfo = async () => {
  ...
  if (data && !Array.isArray(data)) {
    ...
    if (data.mainEngineRpm != 'NaN') {
      engineInfo.value.mainEngineRpm = Number(data.mainEngineRpm).toFixed(1)
    ...
    if (data.mainEngineLoad != 'NaN') {
      engineInfo.value.mainEngineLoad = Number(data.mainEngineLoad).toFixed(1)
    } else {
    ...
    if (data.generatorEngineRpm != 'NaN') {
      engineInfo.value.generatorEngineRpm = Number(data.generatorEngineRpm).toFixed(1)
    ...
    if (data.generatorEngineLoad != 'NaN') {
      engineInfo.value.generatorEngineLoad = Number(data.generatorEngineLoad).toFixed(1)
    } else {
